// Dump raw record bytes to find seq field
...
function dumpRecord(buffer, index) {
  console.log(`\n=== Record ${index} ===`);
  ...
  while (pos < buffer.length) {
    ...
    if (wireType === 2) { // Length-delimited
      ...
      if (/^[\x20-\x7E]+$/.test(str)) {
        ...
        value = `bytes[${length}]: ${data.toString('hex')}`;
      }
    } else if (wireType === 1) { // 64-bit
      if (pos + 8 > buffer.length) break;
      const double = buffer.readDoubleLE(pos);
      const int64 = buffer.readBigInt64LE(pos);
      pos += 8;
      value = `double: ${double} / int64: ${int64}`;
      ...
      const int32 = buffer.readInt32LE(pos);
      pos += 4;
      value = `float: ${float} / int32: ${int32}`;
    } else {
      value = `unknown wireType ${wireType}`;
      break;
    }
    
    console.log(`  Field ${fieldNum} (wire ${wireType}): ${value}`);
  }
}

async function test() {
  const payload = buildPayload('VN30F2512', 5); // Only 5 records
  ...
  while (offset < buffer.length) {
    ...
    console.log('\n=== gRPC Frame ===');
    console.log('Compressed:', compressed, 'Length:', msgLength);
    
    // Parse wrapper to find field 4 records
    let pos = 0;
    while (pos < msgBytes.length) {
      ...
      if (wireType === 2) {
        ...
        if (fieldNum === 4 && recordIndex < 3) {
          dumpRecord(data, recordIndex++);
        }
      } else if (wireType === 0) {
